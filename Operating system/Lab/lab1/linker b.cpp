...
//             return false;
//     }
//     tmp_token = tokenList.at(tokenpos);
//     printf("%d pos\n", tokenpos);
//     printf("%d list size\n", tokenList.size());
...
//     tokenpos++;
//     printf("%s cstr\n", cstr);
//     lineoffset = tmp_token - cstr + 1;
//     printf("lline: %d ineoffset: %d\n", linenum, lineoffset);
//     token = tmp_token;
//     return true;
//     // if (lineoffset == 0)
//     // {
//     //     tmp_token = strtok(cstr, " \t");
//     //     // printf("%s\n",tmp_token);
...
void transAbs(char addressmode, int instruction, int instcount)
{
    ...
    else if (addressmode == 'E')
    {
        if (operand < useOrder.size())
        {
            ...
            // printf("%d:%s=%d+%d\n", operand, key.c_str(), symbolList[key], base);
    ...
    try
    {
        while (!a_file.eof() && !scanEnd())
        {
            ...
            for (int i = 0; i < defcount; i++)
            {
                string sym = readSym();
                int val = readInt();
                // printf("symbol sdsd %s\n", sym.c_str());
                // printf("symbol sdsd %d\n", val);
            ...
            // printf("thisis usecount %d\n", usecount);
            for (int i = 0; i < usecount; i++)
            {
                string sym = readSym();
                // printf("use sym sdsd %s\n", sym.c_str());

                // we don’t do anything here
            }
            // nextline();
            int instcount = readInt();
            // printf("thisis instcount %d\n", instcount);
            num_instr += instcount;
            if (num_instr > 512)
            {
            ...
            for (int i = 0; i < instcount; i++)
            {
                ...
                // printf("addressmode %c\n", addressmode);
                // printf("operand sd %d\n", instruction);
            }
            // printf(defineList.size());
            for (int i = 0; i < defineList.size(); i++)
            {
                string key = defineList.at(i);
                //  printf("base:::%d\n",base);
                //     printf("syli:::%d",symbolList[key]);
